    ...
    .controller('ConfirmOrderPaymentController', ['$scope', '$http', function ($scope, $http) {
        ...
        var services = {
            getOrders: function () {
                ...
                $http.post('/Order/getOrderForConfirmPayments', $scope.orders).then(function (result) {
                    if (result.data == null) {
                        window.location = '../Order';
                    } else {
                        $scope.payments = result.data;
                        for (var r = 0; r < $scope.payments.length; r++) {
                            $scope.payments[r].payment.paymentDate = convertCTOJSDate($scope.payments[r].payment.paymentDate);
                        }
            ...
            confirmOrder: function (data) {
                ...
                $http.post('/Order/confirmOrder', data).then(function (result) {
                    if (result.data) {
                        var index = $scope.payments.indexOf(data);
                        if (index !== -1) {
                            $scope.payments.splice(index, 1);
                            var orderIndox = $scope.orders.findIndex(m => m.code == data.orderCode);
                            if (orderIndox !== -1) {
                                $scope.orders.splice(orderIndox, 1);
                                services.setOrdersStorage($scope.payments);
                            }
                        }
