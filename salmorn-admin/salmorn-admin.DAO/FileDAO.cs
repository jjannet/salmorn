...
namespace salmorn_admin.DAO
{
    public class FileDAO : BaseDAO
    {
        ...
        {
            return context.L_FileUpload.FirstOrDefault(m => m.id == id);
        }

        public L_FileUpload getFileByName(string fileName)
        {
            return context.L_FileUpload.FirstOrDefault(m => m.fileName == fileName);
        }
        ...
        {
            var data = context.L_FileUpload.FirstOrDefault(m => m.id == id);
            if (data != null)
            {
