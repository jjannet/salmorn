...
namespace salmorn_admin.DAO
{
    public class ShippingDAO : BaseDAO
    {
        ...
        {
            return context.T_Shipping.FirstOrDefault(m => m.id == id);
        }
        ...
        {
            var data = context.T_Shipping.FirstOrDefault(m => m.id == item.id);
            if (data != null)
            {
        ...
        public void finishShipping(List<int> ids)
        {
            ...
            {
                var item = context.T_Shipping.FirstOrDefault(m => m.id == i);
                if (item != null)
                {
